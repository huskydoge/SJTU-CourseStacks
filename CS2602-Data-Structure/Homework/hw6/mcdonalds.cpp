...
int main(){
    ...
    long t = 0; // 当前瞌睡值
    long k = 0; // 瞌睡阈值
    long solved = 0; // 记录做出了多少
    cin >> n >> t >> k;

    long texts[1000001]; // 储存每篇文章的瞌睡值
    for(long i = 0 ; i < n; ++ i){
        cin >> texts[i];
    }
    ...
    while(t < k && solved < n){  // 循环条件要清楚！这里下标条件容易落下！
        if((t + texts[solved]) >= k){
            break;
        }
        t = t + texts[solved];
    ...
    double actualSolved = 0;
    actualSolved = solved + (n - solved) * 0.1;

    long score = 100 * actualSolved / n;
